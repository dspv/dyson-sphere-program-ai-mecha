...
Load and visually confirm a disposable ordinary save copy before using the experiment runner. The default provider uses an existing ChatGPT sign-in in Codex CLI; check it with `codex login status` in WSL. From native Windows PowerShell, set `$env:PYTHONPATH` to the checkout's `src\Agent` directory, then run `python -m dsp_agent experiment-once --codex-command "wsl.exe --exec /home/ds/.local/bin/codex" --data-dir "$env:LOCALAPPDATA\DSPAgent\experiments"`. The `--codex-command` path is specific to this WSL installation; if native Windows Codex is signed in, the default `codex` command works instead. The default action set permits only a read-only `inspect` experiment; add `--allow-game-write` for one guarded walk or one-to-five-ore mine after unpausing the game. The command asks Codex for one goal and one tactical plan, then records one attempt. Its local ledger and raw snapshots stay outside Git. A rejected stale snapshot is a recorded failed primitive attempt; an unknown or partial attempt needs operation-ID reconciliation before another write. A subscription-backed goal and read-only inspection succeeded in the paused copied save. The deterministic-stub live mining result in [build status](../.ai/14-build-status.md) verifies the game adapter, not model judgment. The optional `--provider responses --model <configured-model>` path uses the separate Responses API and requires `OPENAI_API_KEY`.
