...
Load and visually confirm a disposable ordinary save copy before using the experiment runner. From native Windows PowerShell, set `$env:PYTHONPATH` to the checkout's `src\Agent` directory and set `OPENAI_API_KEY` locally. Then run `python -m dsp_agent experiment-once --model <configured-model> --data-dir "$env:LOCALAPPDATA\DSPAgent\experiments"`. The default action set permits only a read-only `inspect` experiment; add `--allow-game-write` only for one guarded walk or one-to-five-ore mine. The command makes at most two Responses API calls, one for the goal and one for the tactical plan, then records one attempt. Its local ledger and raw snapshots stay outside Git. A rejected stale snapshot is a recorded failed primitive attempt; an unknown or partial attempt needs operation-ID reconciliation before another write. The CLI has only a no-key smoke check and offline write-boundary test; no paid model call has yet been run. The deterministic-stub live mining result in [build status](../.ai/14-build-status.md) verifies the game adapter, not model judgment.
