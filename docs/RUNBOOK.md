...
This runbook covers the Windows installation, observer, bounded walking and mining, and one guarded Arc Smelter confirmation. Current validation limits are in [build status](../.ai/14-build-status.md).
...
Copy `src/DspAgentBridge/bin/Debug/net472/DspAgentBridge.dll` to `$game\BepInEx\plugins\DspAgentBridge\`. Start DSP through Steam with `Start-Process 'C:\Program Files (x86)\Steam\steam.exe' -ArgumentList '-applaunch 1366540'`; launching `DSPGAME.exe` directly failed Steam initialization in this environment. The installed bridge log identifies `0.6.0`. Query from **native PowerShell**: `Invoke-RestMethod http://127.0.0.1:38741/v1/health`, `Invoke-RestMethod http://127.0.0.1:38741/v1/observe`, and `Invoke-RestMethod 'http://127.0.0.1:38741/v1/entity?entity_id=3'` when exact entity 3 is known in the current session. `python -m dsp_agent build-preview` is read-only. WSL's own loopback did not reach the Windows listener. Native Windows Python can also run `python -m dsp_agent observe` or `python -m dsp_agent entity --entity-id 3` with `PYTHONPATH` set to the checkout's `src\Agent` directory. Health status `stage_c_experimental` names the current scope; it is not a claim that autonomous construction works. A `not_loaded` observation is expected at the menu. `embedded_save_name` is not a reliable loaded filename.
...
## One model-directed experiment

Load and visually confirm a disposable ordinary save copy before using the experiment runner. The default provider uses an existing ChatGPT sign-in in Codex CLI; check it with `codex login status` in WSL. From native Windows PowerShell, set `$env:PYTHONPATH` to the checkout's `src\Agent` directory, then run `python -m dsp_agent experiment-once --codex-command "wsl.exe --exec /home/ds/.local/bin/codex" --data-dir "$env:LOCALAPPDATA\DSPAgent\experiments"`. The `--codex-command` path is specific to this WSL installation; if native Windows Codex is signed in, the default `codex` command works instead. The default action set permits a read-only summary inspection or exact nearby-entity inspection; add `--allow-game-write` for one guarded walk or one-to-five-ore mine after unpausing the game. `experiment-once` asks Codex for one goal and one tactical plan, then records one attempt. `experiment-run` uses the same options and runs up to `--max-attempts 3` by default, with an enforced maximum of five; it stops after a non-achieved verdict or an unresolved error. Ctrl+C stops the local process. Its local ledger and raw snapshots stay outside Git. A rejected stale snapshot is a recorded failed primitive attempt; an unknown or partial attempt needs operation-ID reconciliation before another write. Subscription-backed model-selected exact entity reads and a one-ore mining action succeeded on a copied save; the checked counts and save identity are in [build status](../.ai/14-build-status.md). The optional `--provider responses --model <configured-model>` path uses the separate Responses API and requires `OPENAI_API_KEY`.

### Visible live worker

On this Windows installation, `scripts/run-live-agent.ps1 -ExpectedSession <fresh-session-id> -MaxAttempts 12 -MaxMinutes 20` runs repeated one-attempt decisions against the already loaded copy. Start it with native PowerShell and leave DSP unpaused. `scripts/live-agent-status.py` runs with Windows `pythonw.exe` and opens a small always-on-top window showing the current state and the last completed action. Both use `%LOCALAPPDATA%\DSPAgent\codex-experiments-20260924` for a private ledger, status JSON, console transcript, and `live-agent.stop` file. The worker checks the expected session and planet 102 before every attempt; it stops on pause, a non-achieved result, session change, stop file, elapsed-time limit, or attempt limit. The window's button requests a stop after the current attempt. Esc in DSP pauses the game and interrupts an active walking or mining order. Closing the status window alone does not stop the worker. This worker currently has only inspect, exact-entity read, walk, and bounded mine actions; it cannot build or configure a factory.

## Guarded Arc Smelter confirmation

On a copied save with an Arc Smelter in inventory, select that item through the ordinary UI and move the cursor to a blue `Click to build` preview. Read `python -m dsp_agent build-preview`. Only one active preview with `item_id=2302`, `condition=Ok`, `cover_object_id=0`, and `connection_node=false` is supported. Use its returned `session_id` and exact `position` in `python -m dsp_agent confirm-build --session-id <id> --item-id 2302 --x <x> --y <y> --z <z> --operation-id <new-uuid-hex>`. Poll `python -m dsp_agent build-operation --operation-id <same-uuid-hex>` and compare the item count, recent entity read, and visible world. Reusing the same operation ID returns the prior result. The bridge checks the current preview and game conditions again on the game thread, then calls the game's own `CreatePrebuilds`; invalid or moved previews are rejected. It neither selects a build item nor moves the cursor, sets a recipe, connects power, or proves production. A partial result needs inspection before any retry.
