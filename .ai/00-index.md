...
**Last updated: 2026-09-24. Phase: stages C and D partial.** The project bridge builds against the local Mono game and BepInEx DLLs and has visibly verified stages A and B on copied ordinary saves. On a copy of a new ordinary game, an ordinary UI-built miner, belt, powered sorter, and smelter produced iron ingots over two measured game-time windows; the exact entity counter and visible panel agreed. The bridge confirmed one human-prepared valid Arc Smelter preview into a visible building and rejected a colliding preview; autonomous site selection and line production remain unverified. The agent defaults to the signed-in Codex CLI, and the runner supplies recent checked attempts across goal wording changes. A bounded run selected an exact entity inspection, a five-ore mine, and a walk; checked snapshots showed inventory `2→7` and vein reserve `6,880→6,875`. A visible Windows status window and stop control accompanied a 12-attempt run that reached its cap after six mines, five entity reads, and one walk. A planning-time budget error during an earlier mine was reconciled from the bridge's completed operation and corrected to use its actual start tick. A later UI-prepared smelter confirmation returned `build_conditions_changed`; no building was placed. The optional Responses API adapter remains fake-tested. No autonomous construction, paired strategy trial, or demonstrated learning exists. The [AI-native research](../docs/AI-NATIVE-RESEARCH.md) defines the experimental direction; the exact evidence is in [14-build-status.md](14-build-status.md).
