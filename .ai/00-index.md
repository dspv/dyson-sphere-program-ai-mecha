...
**Last updated: 2026-09-24. Phase: stage C partial implementation and AI-native experiment groundwork.** The project bridge builds against the local Mono game and BepInEx DLLs and has visibly verified stages A and B on copied ordinary saves. On a copy of a new ordinary game, an ordinary UI-built miner, belt, powered sorter, and smelter produced iron ingots over two measured game-time windows; the exact entity counter and visible panel agreed. The bridge confirmed one human-prepared valid Arc Smelter preview into a visible building and rejected a colliding preview; autonomous site selection and line production remain unverified. A goal ledger with observed-context retrieval, a one-attempt runner, and a bridge action adapter completed one bounded iron-mining attempt in DSP using a deterministic planner stub. The one-attempt CLI now defaults to the signed-in Codex CLI; a live model-chosen goal and read-only inspection succeeded on the paused copied save. The optional Responses API adapter remains fake-tested. No model-selected game mutation, paired strategy trial, or demonstrated learning exists. The [AI-native research](../docs/AI-NATIVE-RESEARCH.md) defines the experimental direction; the exact evidence is in [14-build-status.md](14-build-status.md).
