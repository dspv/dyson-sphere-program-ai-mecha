...
`experiments.py` stores the offline goal and experiment ledger. `experiment_runner.py` connects injected observer, goal chooser, tactical planner, executor, and verifier callbacks for one bounded attempt. It records the intent before execution, gives the planner relevant past outcomes, rejects unlisted action kinds, and treats lost action results, failed observation, or a session change as unresolved. The ledger requires an external evidence reference for achieved or failed outcomes, keeps terminal verdicts immutable, blocks new attempts while a partial or unknown attempt awaits reconciliation, and rejects an unchanged retry of a failed experiment in the same state. Retrieval currently matches near-term goal text exactly; it does not rank context or check evidence references itself. `responses_experiment.py` supplies structured goal and plan proposals through the Responses API with local action validation, but it has only mocked HTTP checks. No live model call, game adapter, or verified outcome source is wired to the runner.
